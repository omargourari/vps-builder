...
# linux_init_harden --resetrootpwd
# linux_init_harden --resetrootpwd -q -hide
...
function usage() {
    ...
    echo ""
    echo "Example: bash ./$SCRIPT_NAME.sh --resetrootpwd"
...
case "$OS" in
    ...
    ubuntu)
        ...
        if [[ "$VER" != "16.04" ]] && [[ "$VER" != "18.04" ]] && [[ "$VER" != "18.10" ]] && [[ "$VER" != "20.04" ]] && [[ "$VER" != "21.10" ]]; then
...
function revert_software_installs(){
    ...
    echo
    center_err_text "Error while installing $1"
    center_err_text "This may be a false-alarm"
    center_err_text "Script will continue to next step"
    file_log "The $1 installation failed..."
...
function revert_ssh_only_login(){
    ...
    {
        set_exit_code $(service_action_and_chk_error "sshd" "
        ")
...
STEP_TEXT=(
    ...
    "Reset sources.list to defaults" #3
    "Installing required softwares" #4
    "Installing Java" #5
    "Installing Nginx" #6
    "Installing Python" #7
...
setup_step_start "${STEP_TEXT[4]}"
{
    ...
    apt-get install -y curl fail2ban logwatch unzip wget
    set_exit_code $?

    # Create directories and config file for Logwatch
    mkdir /var/cache/logwatch
    wget -q ${DOWNLOADPATH}files/logwatch.conf -O /etc/logwatch/conf/logwatch.conf
...
{
    file_log "Installing Python"
    apt install -y build-essential libssl-dev libffi-dev python3-dev
    exit_code=echo $(python3 --version | grep "Python 3.8." | wc -l)
    if [[ $exit_code -eq 0 ]]; then
        file_log "Python installed successfully"
    else 
        file_log "Python installation failed"
    fi
...
setup_step_start "${STEP_TEXT[8]}"
{
    ...
    echo "source /etc/zsh/zsh-autosuggestions/zsh-autosuggestions.zsh" > /etc/zsh/zshrc
